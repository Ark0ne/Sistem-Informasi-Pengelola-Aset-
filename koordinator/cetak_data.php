...
$pdf->AddPage();
$pdf->SetTitle('Laporan Koordinator Aktif');
/*
...
$pdf->Cell (20, 1, "Laporan Koordinator Aktif", 0, 1, 'C');
$pdf->ln();
$pdf->Cell(1, 1, '',0,0, 'c');
...
$pdf->Cell(3, 1, 'ID Koordinator', 1, 0, 'C', 1);
...
$query= mysqli_query($konek, "select * from koordinator");//mengambil data dari view guru
while($row = mysqli_fetch_array($query)){ 
    $pdf->Cell(1, 1, '',0, 0, 'c'); 
    $pdf->Cell(3, 1, $row['id_koordinator'], 1, 0, 'C');
    $pdf->Cell(5, 1, $row['nip_koordinator'], 1, 0, 'C'); 
    $pdf->Cell(3, 1, $row['nama_koordinator'], 1, 0, 'C');
    $pdf->Cell(4, 1, $row['instansi_koordinator'], 1, 0, 'C');
    $pdf->Cell(3, 1, $row['kontak_koordinator'], 1, 1, 'C');    
} 
$pdf->ln();
$pdf->SetFont("Arial", "", "10");
